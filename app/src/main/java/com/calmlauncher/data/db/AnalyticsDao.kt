...
interface AnalyticsDao {
    ...
    /**
     * Clear one day's per-app rows before rewriting them. Without this, an app that used to
     * register usage on a day but no longer does keeps its stale row forever — [Upsert]
     * only ever adds or replaces, it never removes.
     */
    @Query("DELETE FROM app_usage WHERE dayStartEpochMs = :dayStartEpochMs")
    suspend fun deleteAppsForDay(dayStartEpochMs: Long)

    /**
     * Clear one day's sessions before re-inserting them. Sessions have an autogenerated id
     * and no natural key, so a plain insert on every refresh would stack up duplicates of
     * the same real-world session.
     */
    @Query("DELETE FROM usage_session WHERE dayStartEpochMs = :dayStartEpochMs")
    suspend fun deleteSessionsForDay(dayStartEpochMs: Long)
